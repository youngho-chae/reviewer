...
- **SNS 채널 연동·검증 (2026-07-10)**: 채널 관리 `/r/me/channels`(연동/해제/본인 인증) — OAuth 코어 `src/lib/sns-oauth.ts`(네이버/Meta/틱톡 로그인, state CSRF·**토큰 미저장**), 키 미설정 시 데모 검증(`/r/channels/verify`, verifiedVia="demo"·실키 시 403). verified는 신뢰 표식 — **참여 게이트는 연동 여부뿐(P1)**. 해제 = 등급 재계산·진행 패스 유지.
