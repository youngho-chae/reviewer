...
| R-02 | 탐색 (**기본 지도 보기** + 리스트 전환 · 기본 정렬 **추천순** · 필터 시트 [SNS 채널·카테고리 '전체' 칩 + **지역** + 초기화/적용하기, draft 커밋] · 홈 선택 지역 `?area=` 수신 → 행정 기준점 반경 3km 포커스 · `?scope=all`(홈 '전체 리스트' 진입) → **전국 축소 시작·시도 클러스터 즉시 노출** · 전국 축소 시 **시도 클러스터**(건수 마커, 탭 줌인) · 반경 3km 필터는 **지도 전용**(리스트는 필터 시트 기준) · 지역 필터 **3상태**(미선택 기본/현위치 토글/지역) · 검색 0건 시 추천 4개) | `/r/explore` (`?mode=list` `?sort=` `?cat=` `?ch=` `?area=` `?loc=me` `?scope=all` `?q=`) — 기본 `mode=map` |
| R-03 | 매장 상세 · 채널 선택 참여 | `/r/store/[id]` |
| R-04 | 내 체험권 (방문형/기자단 세그먼트 · **체험권/리뷰작성 탭 + 상태 칩** — 파생 표시 상태 "제출 기한 초과"·"재제출 기한 초과" 칩 포함, 2026-07-10) | `/r/passes` |
| R-04a | 체험권 상세 (7-상태+파생 상태 분기, QR/코드 입력/리뷰/재제출/취소 — 취소는 "방문을 취소하시겠어요?" 중앙 모달) | `/r/passes/[id]` |
...
| R-07 | MY (프로필·메뉴 허브·약관·회원 탈퇴 · 연동 채널 섹션 → R-15) | `/r/me` |
| R-15 | **채널 관리** (SNS 연동/해제/**재연동(다시 인증)** + 본인 검증 — 프로바이더 OAuth(네이버/페이스북/틱톡 로그인), 키 미설정 시 데모 검증 화면 `/r/channels/verify` — 데모 데이터(표시명·URL·수치) 임시 등록 폼) | `/r/me/channels` (`?connected=` `?error=`) |
| R-17 | **체험 포인트** (잔액·출금 신청(세금 3.3%·수수료 미리보기)·출금/포인트 내역 — 배송형 리뷰 승인 적립, MY 잔액 카드 진입) | `/r/me/points` |
...
| AD-06 | **포인트 출금 처리 큐** (지급 완료/반려 — 신청 시점 확정 세액·실지급 표시) | `/admin/points` |
...
| 버전 | 일자 | 내용 |
|---|---|---|
| 1.4 | 2026-07-12 | **레뷰 벤치마크** — R-17 체험 포인트(`/r/me/points`)·AD-06 출금 큐(`/admin/points`) 신설, 탐색에 배송형 세그먼트(`?tab=delivery`, 리스트 전용)·홈 배송 캐러셀, 사장님 홈 발송 대기 큐. |
| 1.3 | 2026-07-10 | **추가 작업 리스트 반영** — R-02 탐색: 기본 정렬 추천순, 필터 시트 개편(전체 칩·지역 항목·초기화/적용하기 draft 커밋·`?ch=`/`?area=` URL 유지), 홈 지역 `?area=` 연동(행정 기준점 3km 포커스), 전국 축소 시 시도 클러스터, 검색 0건 추천. R-01 홈: 동적 섹션 타이틀("{시군구}에서 갈 수 있어요")·직선 거리 캡션·"참여 중" 뱃지·전체 리스트 추천순 30개. R-04/R-04a: 파생 표시 상태(제출/재제출 기한 초과) 칩·취소 중앙 모달 카피 개편. |
