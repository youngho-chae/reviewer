...
| R-02 | 탐색 (**기본 지도 보기** + 리스트 전환 · 기본 정렬 **추천순** · 필터 시트 [SNS 채널·카테고리 '전체' 칩 + **지역** + 초기화/적용하기, draft 커밋] · 홈 선택 지역 `?area=` 수신 → 행정 기준점 반경 3km 포커스 · `?scope=all`(홈 '전체 리스트' 진입) → **전국 축소 시작·시도 클러스터 즉시 노출** · 전국 축소 시 **시도 클러스터**(건수 마커, 탭 줌인) · 반경 3km 필터는 **지도 전용**(리스트는 필터 시트 기준) · 검색 0건 시 추천 4개) | `/r/explore` (`?mode=list` `?sort=` `?cat=` `?ch=` `?area=` `?scope=all` `?q=`) — 기본 `mode=map` |
