...
// 이미 적용됨(2026-07-06): R-02 72560:2(보드 생성 시 내장) · R-01 72559:2(보드 생성 시 내장) · R-00 72552:2 · R-03 72553:2 · R-04a 72554:2 · O-10 72555:2 · O-02 72556:2 · AD-01 72557:2
...
const GUIDES = {
  "R-02": { boardId: "72573:2",
    ...
    defaults: [
      "Default 상태(코드 확정): mode=리스트 · tab=방문형 · cat=전체 · sort=우리 추천(recommended) · layout=row(1단) · 검색 빈 값 · 지도 핀 선택 없음.",
      "URL 파라미터 진입 시 해당 값으로 초기화(?mode=map이면 지도뷰부터, ?sort=·?cat=은 셀렉트/칩 선택 상태로 시작).",
      "FAB는 리스트뷰=[📍 지도]/지도뷰=[☰ 리스트] 라벨 토글 · BottomNav 탐색 탭 활성.",
    ],
    inter: [
      ["검색 입력 (필터 동작)","타이핑 즉시 결과·헤더 카운트 갱신(클라이언트 필터) — 다른 필터와 동시 적용"],
      ["카테고리 칩 탭 (필터 동작)","단일 선택 전환 → 리스트/핀 즉시 갱신 · '전체' 탭 시 해제 효과"],
      ["정렬 셀렉트 변경","5종 중 선택 → 리스트 재정렬(스크롤 최상단 복귀)"],
      ["방문형/기자단 탭","카드 데이터셋 전환 — 검색·카테고리·정렬 상태 유지"],
      ["[리스트/그리드] 토글","row(1단)↔grid(2단) 레이아웃 전환 — 데이터·스크롤 위치 유지"],
      ["FAB 탭","리스트↔지도 모드 전환(같은 라우트) · 필터 상태 그대로 승계"],
      ["지도 핀 탭","토스트 카드 슬라이드 업 + FAB 위로 양보 · 빈 곳 탭=카드 닫힘 · [상세→]=/r/store/[id]"],
      ["카드 탭(리스트)","/r/store/[id] — [P1] 모든 카드 참여 가능(등급 게이트 없음)"],
    ],
    variants: [
      ["V1 ★Default","리스트 · row","검색/필터 기본 + RowCard 리스트(라벨 칩 3종·개인화 금액)"],
      ["V2","리스트 · grid","2단 GridCard + 잔여 ≤3 pill"],
      ["V3","기자단 탭","기자단 카드 리스트(정산 예정금·원천징수 문구)"],
      ["V4","검색/필터 무결과","\"다른 동네 찾아볼까요?\" 빈 상태"],
      ["V5","캠페인 0건","\"지금은 동네가 잠깐 쉬는 중\" 빈 상태"],
      ["V6","지도뷰 기본","풀스크린 지도+핀+상단 오버레이+FAB [☰ 리스트]"],
  ...
  "R-01": { boardId: "72572:2",
    data: [
      ...
      ["매장 카드","4:3 실사진 · 매장명(2~20자, 1줄 말줄임) · 카테고리+지역 · 금액 ₩bold(개인화=연동 채널 중 내 최대 혜택) · 채널 아이콘 블/인/틱 1~3개 · ★평점(소수1)","사진 없음 케이스 없음(카테고리 기본 이미지) · 채널 미연동 금액=기준 지원금 · [P1] 등급 오버레이/잠금 없음"],
    ...
    inter: [
      ...
      ["매장 카드 탭","/r/store/[id] 상세 이동 — [P1] 모든 카드 참여 가능"],
      ["종 아이콘 탭","/r/notifications — 미읽음 dot 소멸은 알림함에서 읽음 처리 후"],
      ["[혜택 보기] 탭","/r/rewards"],
      ["하단 등급 배너 탭","/r/grade — 카피 \"한 등급 오르면 받는 게 확 커져요\" (혜택 크기 소구)"],
    ],
    variants: [
      ["V1 ★Default","기본(연동 A등급)","개인화 금액·채널 아이콘 리스트 — [P1] 오버레이 없음"],
      ["V2","미연동(N등급)","전 카드 금액=기준 지원금(연동 채널 없음) — 참여는 전부 가능"],
      ["V3","GPS 폴백","헤드라인 지역=첫 매장 지역(권한 프롬프트 무시/거부 후)"],
      ["V4","빈 상태","\"지금은 동네가 잠깐 쉬는 중\" — 큐레이션·그리드 대신 빈 카드"],
      ["V5","미읽음 알림","종 아이콘 brand dot 상태"],
    ],
    fixedFree: "고정(변경 금지): B급 카피 원문·큐레이션 3종 필터 매핑·정렬 규칙 2종(도보/혜택 큰 순)·최대 4개 제한·개인화 금액 로직·[P1] 등급 게이트 표현 금지. 자유(재량): 타일 파스텔 팔레트·카드 그리드 비주얼·배너 일러스트·스티커 칩 각도.",
  },
  ...
  "R-03": { boardId: "72574:2",
    ...
    inter: [
      ...
      ["모달 [발급받고 체험권 보기]","라벨 \"발급 중...\" 비활성 → 성공 시 체험권 상세(R-04a) 이동 · 실패 시 모달 내부 하단 에러 원문(M5~M7)"],
      ["모달 배경 탭 / [취소]","모달 닫힘 — 선택 상태·스크롤 유지"],
    ],
    variants: [
      ["V1 ★Default","기본(참여 가능)","채널 연동 OK — CTA 파랑 \"참여하기\" 활성, 금액 카드 정상 · [P1] 등급 무관 참여 가능"],
      ["V2","미연동 채널 선택","금액 카드=연동 안내 문구 · CTA \"{채널} 미연동\" 비활성"],
      ["V3","잔여 0 · 마감","다크 히어로 잔여 \"0매\" · CTA \"마감되었습니다\" 비활성 — 정보 열람은 그대로"],
      ["V4","이미 참여","CTA가 링크 버튼 \"내 체험권 보기 →\"(파랑 활성)로 교체"],
      ["V5","참여 확인 모달","하단 시트 — 요약 3행+고지+버튼 2"],
      ["V6","키워드 없음","'후기에 꼭 강조해주세요' 섹션 숨김(소개만)"],
    ],
    fixedFree: "고정(변경 금지): CTA 4분기 카피(참여하기/마감/미연동/내 체험권)·Default 채널 우선순위·금액 계산식(등급 배율)·부담 주체 고지·네이버 평점 출처 표기·[P1] 등급 게이트 표현 금지. 자유(재량): 히어로/카드 비주얼·칩 형태(단 미연동 🔒 시각 구분 유지)·모달 스타일·섹션 간 배경 리듬.",
  },
  ...
  "O-10": { boardId: "72541:2",
    data: [
      ...
      ["모집 현황 카드","플랜명(Free/Basic/Standard/Premium) · 월 한도(5/15/50/100 — 무제한 폐기 2026-07-31) · 사용 N팀 · 보너스 +N팀(있을 때만) · 잔여 N팀 — 전부 정수","잔여 0 → 빨간 카피 + [캠페인 생성] 비활성(V3)"],
      ["총 모집 인원","numeric · 최소 1 · 예 12 → 자동 분배 결과 안내(전 플랜 균등: S3 A3 B3 C3 — 확정 정책 8-3)","0 이하 입력 시 제출에서 에러 M19"],
