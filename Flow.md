...
```
        ...
        │
        │  BottomNav (5탭, v2.9 재구성)
        ├──── 홈 (home)         → /r/home    (큐레이팅)
        ├──── 탐색 (navigation) → /r/explore (리스트+지도)
        ├──── 등급 (trophy)     → /r/grade   (R-06 — 등급 배지+진행도+3-stat+30일 성과+등급별 혜택+체험권 entry)
        ├──── 혜택 (ticket)     → /r/rewards (R-06b — viral 박스+카운터+내 보상+보낸 초대)
        └──── MY (user)         → /r/me
                                  ├ 알림함 (header 종)
                                  │   → /r/notifications
                                  └ 로그아웃

  ※ /r/passes 진입 = 홈/탐색/등급 헤더 카드 + 매장 상세 sticky CTA 응답으로
  ※ 등급↔혜택 분리: 등급 탭 = 지금 받을 수 있는 정적 혜택, 혜택 탭 = 친구와 함께 키우는 동적 보상
```
...
|---|---|---|
| 체험자 BottomNav | 5탭 (홈/패스/등급/MY/+) | **v2.9 5탭 홈/탐색/등급/혜택/MY** (v2.0 홈/체험권/등급/MY → v2.6 4탭 홈/탐색/혜택/MY → v2.9 등급·혜택 분리 후 5탭). 체험권은 홈·탐색·등급 헤더 카드에서 entry. 알림은 헤더 종 아이콘 |
