...
4. **방문형 첫 사용**: `/r/explore` → 매장 카드 → `/r/store/[id]` → [참여하기] → `/r/passes/[id]` (active, QR + 캠페인 4자리 코드 표시)
...
5. **사용 처리 (QR 또는 4자리)**:
   `/o/scan` → 체험권 화면의 캠페인 4자리 입력(또는 QR 스캔) → [조회] → 결제액 입력 → [사용 처리] → `/o/home` 복귀
   - 캠페인 생성 시 사장님이 지정한 4자리(`useCode`)가 체험자 화면에 노출됨
6. **후기 모니터링**:
