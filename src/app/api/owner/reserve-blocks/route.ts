...
export async function POST(req: NextRequest) {
  ...
  const { campaignId, action, date, time } = await req.json();
  ...
  switch (action) {
    ...
    case "block_slot": {
      if (!/^\d{4}-\d{2}-\d{2}$/.test(d) || d < today || !campaignTimeSlots(scheduleOf(c)).includes(t)) {
        return NextResponse.json({ error: "차단할 날짜와 시간을 선택해주세요" }, { status: 400 });
      }
      if (!(blocks.slots ?? []).some((x) => x.date === d && x.time === t)) {
        blocks.slots = [...(blocks.slots ?? []), { date: d, time: t }].sort((a, b) =>
