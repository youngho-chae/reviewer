...
// S+의 추가 혜택은 배율 외 영역(프로모션 최우선·전용 배지 — §10.6.
// 포인트 보너스 표기는 2026-08-18 제거: 혜택/포인트 = Out of scope, UI 비노출).
const BENEFITS: { g: Grade; d: string; amt: string }[] = [
  { g: "S+", d: "모든 캠페인 참여 · 기준 지원금 전액 · 프로모션 최우선", amt: "100% 지원금" },
...
const TIER_REQUIRE: Record<Grade, string> = {
  ...
  // 2026-08-18 문구 정정 — N은 "연동 조건"이 아니라 "채널 미연동 상태" 자체 (혼란 방지)
  N: "채널이 연동되지 않은 상태 (연동하면 바로 C 등급부터 시작)",
};
        ...
        <section className="px-5 mt-3">
          <div className="rounded-md border border-hairline bg-canvas p-4">
            ...
            <p className="mt-3 pt-3 border-t border-hairlineSoft text-[11px] text-muted leading-[1.5]">
              S+ 혜택: 프로모션 최우선 · 골드 배지. 무리한 결제가 아니라 꾸준하고 성실한 완료가 핵심이에요.
            </p>
          </div>
        </section>
      )}
