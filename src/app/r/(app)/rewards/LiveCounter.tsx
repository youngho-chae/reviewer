  ...
  return (
    <div className="rounded-md bg-tile1 text-white px-4 py-3 flex items-center gap-3">
      ...
      <div className="text-[12px] leading-[1.45] flex-1 min-w-0">
        오늘 열린 박스 <span className="font-semibold">{sbNum(SBUI.liveCount, c.todayBoxCount.toLocaleString())}</span>개
        <div className="text-[11px] text-white/70 mt-0.5 truncate">
          최근: {top.nickname} — <strong className="text-[#FDE047]">{sbNum(SBUI.reward, top.rewardText)}</strong>
          <span className="opacity-60"> · {sbNum(SBUI.matrix, top.matrix)}</span>
        </div>
      </div>
    </div>
  );
