 ...
 * 캠페인명(2줄) → 매장명 → 타일 2개(방문 예정|예약 확인 필요 · 사용 완료 n/총).
      ...
      <div className="px-5 mt-3 space-y-3">
        {visible.map((it) => (
          <Link key={it.id} href={`/o/campaign/${it.id}`} className="cp-action block rounded-lg border border-hairline bg-canvas p-4">
            ...
            <div className="mt-2.5 text-[16px] font-bold text-ink leading-[1.35] line-clamp-2">{it.title}</div>
            <div className="mt-0.5 text-[13px] text-muted">{it.storeName}</div>
