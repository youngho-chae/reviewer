...
//   2) 리뷰 기한(72h) 초과: used 상태 방치 → 노쇼 카운트 + 양측 알림 (1회)
...
export const REVIEW_DEADLINE_MS = 72 * 60 * 60 * 1000;
...
export function sweepPassLifecycle(db: DBShape, now: number = Date.now()): boolean {
  ...
  for (const p of db.passes) {
    ...
    // 2) 리뷰 기한(72h) 초과 미제출 — 노쇼 1회 반영 + 양측 알림
    if (p.status === "used" && p.usedAt && now > p.usedAt + REVIEW_DEADLINE_MS && !p.overdueHandled) {
      ...
      db.notifications.push({
        ...
        role: "reviewer",
        title: "리뷰 기한 초과",
        body: `${store?.name ?? "매장"} 리뷰 제출 기한(72시간)이 지났습니다. 반복 시 캠페인 참여가 제한될 수 있습니다.`,
        createdAt: now,
        read: false,
        link: `/r/passes/${p.id}`,
      });
      db.notifications.push({
        ...
        role: "owner",
        title: "리뷰 미제출 안내",
        body: `체험 완료 후 72시간 내 리뷰가 제출되지 않은 체험권이 있습니다. 운영팀이 이력을 관리합니다.`,
        createdAt: now,
        read: false,
        link: "/o/reviews",
      });
    ...
    // 3) 만료 임박(6시간 전) 리마인드 — 방문형(24h 기한)만
    if (
      p.status === "active" &&
      !p.expiringSoonNotified &&
      p.expiresAt - now > 0 &&
      p.expiresAt - now <= EXPIRY_REMINDER_MS &&
      p.expiresAt - p.issuedAt <= 25 * 60 * 60 * 1000 // 기자단(캠페인 종료 기한) 제외
    ) {
