...
//  - overdue          : used인데 리뷰 제출 기한(이용 후 7일)이 지남 → 제출 CTA 제거
...
export function passDisplayStatus(
  p: Pick<Pass, "status" | "usedAt" | "overdueHandled" | "rejectedAt" | "resubmitCount">,
  now: number = Date.now(),
): PassDisplayStatus {
  if (p.status === "used") {
    const deadline = p.usedAt ? p.usedAt + REVIEW_DEADLINE_MS : null;
