...
//  - 구매 제한(초기): Basic 1회 / Standard 1회 / Premium 2회 per 결제 주기
//    → Premium 2회 초과 수요 = 대량 모집(Enterprise) 플랜 전환 대상 (운영팀 문의 안내)
//
...
// 결제 주기당 구매 가능 횟수 (전략안 §7 초기 권장안 — Free는 미판매)
export const REFILL_MAX_PER_CYCLE: Record<PlanKey, number> = {
  Free: 0,
  Basic: 1,
  Standard: 1,
  Premium: 2,
...
// 사장님의 리필 구매 가능 상태 (구매 API·UI 공유)
export function refillPurchaseState(db: DBShape, owner: Owner, now: number = Date.now()) {
  const used = refillsThisCycle(db, owner.id, now).length;
  const max = REFILL_MAX_PER_CYCLE[owner.plan];
  return {
    plan: owner.plan,
    grant: refillGrantFor(owner.plan),
    price: REFILL_PRICE,
    purchasedThisCycle: used,
    maxPerCycle: max,
    canBuy: owner.plan !== "Free" && used < max,
  };
}
