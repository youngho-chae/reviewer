...
## 시나리오 J — 체험자·사장님: 예약형 방문 (2026-07-16 리뷰노트 벤치마크 · v2 제안 플로우)

> 정본: 운영정책서 §15 · `src/lib/reservation.ts` — 예약은 선정이 아니라 일정 조율(발급 즉시 · 사장님 일방 거절 없음 · **확정 전 QR 미노출**)

1. (체험자) 탐색 "📅 예약 필수" 배지 카드 → 상세 예약 안내 배너(가능 요일·시간 `reservationNote`) 확인 → [예약하고 체험권 받기].
2. 신청 시트에서 채널 선택 + **희망 방문 일시 선택**(날짜: 오늘~14일·캠페인 종료일 한도, 시간: 11:00~20:30 30분 슬롯) → 발급 완료. 체험권 유효기간 = **예약일 당일 말**(72h 예외), 예약 상태 = "예약 확인 대기" — **확정 전에는 체험권 상세에 QR 대신 예약 대기 화면**(예약 변경·참여 취소 가능)이 노출된다.
3. (사장님) 홈 "📅 방문 예약 N건" 큐 — 익명 #last4·일시·캠페인명(등급·실명 비노출)에서 둘 중 선택:
   - **[예약 확인]** → 확정 → 4로.
   - **[다른 시간 제안]** → 대안 슬롯 최대 3개 + 수기 안내사항(선택지가 더 필요하거나 추가 안내 시 직접 작성 — 체험자에게 원문 노출) → "제안함 · 응답 대기".
4. (체험자·제안 수신 시) "사장님이 다른 방문 시간을 제안했어요 📅" 알림 → 체험권 상세에서 **협상 히스토리 타임라인**(희망→제안, 안내사항 원문) 확인 후 **라디오 최대 4행**(제안 슬롯 최대 3 + 기타 직접 입력) 중 선택:
   - 슬롯 수락 → **즉시 예약 확정 + 체험권(QR) 활성화** + 사장님 알림.
   - 기타 일시 직접 입력 = **재제안(1회만)** → "예약 확인 대기"로 재요청(QR 계속 잠김) → 사장님은 히스토리를 보고 **[예약 확인] 또는 [거절]만** 가능(제안 재사용 불가 — 서로 각 1회). 사장님 거절 = 신청 취소(슬롯 복구·**패널티·12h 재신청 제한 없음**).
   - **거절 → 이용 취소** — 슬롯 복구, **패널티·12h 재신청 제한 없음**(언제든 재신청 가능).
5. (체험자·확정 후) 체험권 카드·QR 화면에 예약 일시+"예약 확정" 칩. 일정이 바뀌면 예약 패널에서 **예약 변경**(확인 대기 복귀 — QR 다시 잠김·기한 재계산, 사장님 알림).
6. 예약 시간 방문 → QR 제시/코드 입력(확정 후에만 가능 — 서버도 검증) → 이후 방문형과 동일(리뷰 7일). 미방문 시 예약일이 지나면 만료(슬롯 복구·노쇼 카운트 — 기존 스윕).
