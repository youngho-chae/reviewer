...
[/r/home] 홈 (발견 전용 — 리스트/필터/지도는 /r/explore에서) · 디자인 시스템 v2
    · 상단 바: 📍 {지역명}(GPS reverse-geocode, 실패 시 첫 매장 지역 폴백) ⌄ + 검색·알림 아이콘
    · 프로모 배너(블루 마케팅 슬롯) + 내 활동 3-stat(이번 달 아낀 금액 / 밀린 리뷰 / 🎫 참여 예정 → /r/passes)
    · "걸어서 갈 수 있어요👀" 2단 카드(최대 4개, 도보 순) + "내가 체험할 수 있는 전체 리스트👀"(혜택 큰 순)
    · 카드 = 4:3 사진 + SNS 배지(블로그/인스타/틱톡) + 🎫 N 남음 + 가게명 + "최대 ₩N 지원"
    · [P1] 등급 부족 오버레이 없음 — 모든 카드 참여 가능, 금액만 내 채널 등급 기준 개인화

[/r/explore] 탐색 (리스트 + 지도 통합) · 디자인 시스템 v2
    · 헤더 = [방문형 | 기자단] 세그먼트 타이틀 + 검색·알림 아이콘
    · 카테고리 아이콘 칩 6그룹(선택=검정 보더) + 필터 버튼 · "근처 체험 N개 발견!" + 정렬방식 ⌄(5종)
    · experience-row: 96px 썸네일 + SNS 배지 + 가게명 + 카테고리·거리 + "최대 ₩N 지원" + 우상단 🎫 N 남음
    · 하단 중앙 검정 pill FAB [📍 지도 보기] ↔ [☰ 목록 보기]
[/r/explore?mode=map] 지도 — 풀맵 + 2줄 흰 pill 마커(매장명/최대 금액, 선택=퍼플 강조)
    · 핀 미선택 시 바텀시트(카테고리 칩+결과 카운트+리스트) · 핀 탭 → 하단 요약 카드(탭 → 상세)

[/r/store/[id]] 매장 상세 · 채널 선택 참여 · 디자인 시스템 v2
    · 4:3 풀블리드 사진 + SNS 배지 + 가게명 + 플레이스 URL 링크
    · info-strip 3분할(체험 마감일 / 리뷰 마감 기한(이용 후 72시간) / 🎫 잔여) + 퍼플 안내 배너("발급 후 24시간 내 미사용 시 소멸")
    ...
    · cta-bar(하단 고정): 지원 금액 ₩N + [체험권 발급받기]
    → POST /api/passes { campaignId, channel }
        · [P1] 등급 자격 검증 없음 — 채널 연동·잔여 슬롯만 확인. 선택 채널 등급으로 지원금 확정, quota 버킷 차감(consumedSlot 기록) · Pass 생성(active, 24h TTL) · 사장님 알림
        → /r/passes/{passId}
