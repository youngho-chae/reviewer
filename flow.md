...
## 시나리오 J — 체험자·사장님: 예약형 방문 (2026-07-16 리뷰노트 벤치마크)

> 정본: 운영정책서 §15 · `src/lib/reservation.ts` · docs/벤치마크-리뷰노트.md — 예약은 선정이 아니라 일정 조율(P1: 거절 없음, 발급 즉시)

1. (체험자) 탐색 "📅 예약 필수" 배지 카드 → 상세 예약 안내 배너(가능 요일·시간 `reservationNote`) 확인 → [예약하고 체험권 받기].
2. 신청 시트에서 채널 선택 + **희망 방문 일시 선택**(날짜: 오늘~14일·캠페인 종료일 한도, 시간: 11:00~20:30 30분 슬롯) → 발급 완료. 체험권 유효기간 = **예약일 당일 말**(72h 예외), 예약 상태 = "예약 확인 대기".
3. (사장님) 홈 "📅 방문 예약 N건" 큐 — 익명 #last4·일시·캠페인명(등급·실명 비노출) → [예약 확인] → 확정.
4. (체험자) "예약이 확정되었어요 📅" 알림 — 체험권 카드·QR 화면에 예약 일시+"예약 확정" 칩. 일정이 바뀌면 QR 화면 예약 패널에서 **예약 변경**(확인 대기로 복귀·기한 재계산, 사장님에게 변경 알림).
5. 예약 시간 방문 → QR 제시(예약 미확정이어도 사용 가능 — 게이트 아님) → 이후 방문형과 동일(리뷰 7일). 미방문 시 예약일이 지나면 만료(슬롯 복구·노쇼 카운트 — 기존 스윕).
