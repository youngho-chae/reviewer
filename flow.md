...
[/r/home] 홈 (발견 전용 — 리스트/필터/지도는 /r/explore에서) · 디자인 시스템 v2
    · 상단 바: 📍 {현재 위치}(GPS reverse-geocode, 실패 시 첫 매장 지역 폴백) ⌄ + 검색·알림 아이콘
      — 위치를 누르면 지역 변경 바텀시트(?area=) → '걸어서 갈 수 있어요'만 해당 지역 기준으로 변경(전국 리스트는 무관)
...
[/r/explore] 지도(기본) — 풀맵 + 2줄 흰 pill 마커(매장명/최대 금액, 선택=퍼플 강조)
    · 핀 미선택 시 바텀시트(칩+결과 카운트+리스트) · 핀 탭 → 하단 카드 캐러셀(좌우 스와이프 = 거리순 다음 매장)
