# CATCHPASS · 디자인 가이드 (Reference)

> 본 문서는 **Airbnb 디자인 시스템 역분석**을 그대로 옮긴 참조 자료입니다.
> CATCHPASS 자체 디자인 시스템(컬러 토큰, 타이포, 컴포넌트)을 정의하기 전,
> "사진 중심의 마켓플레이스 + 단일 강조색 + 부드러운 라운드 + 한 단계 그림자"라는
> 기준 사례로 활용합니다.
>
> CATCHPASS 적용 시 주의:
> - 강조색은 Rausch(#ff385c)가 아닌 CATCHPASS 브랜드 컬러로 치환 필요
> - "단일 강조색 · 흰 캔버스 · 인크 텍스트" 원칙은 그대로 차용
> - 사진/카드 중심 마켓플레이스 패턴은 매장/캠페인 카드에 그대로 적용 가능
> - 한 단계 그림자 + 라운드 일관성 룰은 그대로 도입 권장
...
## CATCHPASS 적용 시 매핑 가이드 (TODO)

> 위 Airbnb 토큰을 그대로 쓰지 않습니다. CATCHPASS 자체 토큰을 정의할 때 아래 항목을 채워 넣으세요.

| Airbnb 토큰 | CATCHPASS 대응 | 비고 |
|---|---|---|
| `colors.primary` (#ff385c Rausch) | TBD — CATCHPASS 브랜드 컬러 1종 | 등급 배지·CTA·QR 액센트 단일 강조색으로 사용 |
| `colors.ink` (#222222) | 그대로 #222222 권장 | 본문/제목 통일 |
| `colors.canvas` (#ffffff) | 그대로 흰 캔버스 | 다크모드는 v2 이후 검토 |
| `typography.display-xl` (28px/700) | 홈 히어로 카드 등급 배지·정산금 숫자 | 카피보다 숫자에 무게 |
| `typography.rating-display` (64px/700) | 매장 상세 ★ 평점·정산금 메인 숫자 | "가장 큰 한 곳"에만 |
| `rounded.md` (~14px) | 매장 카드·체험권 카드 라운드 | 일관 적용 |
| `rounded.full` | QR 영역·등급 배지·하트 | 원형 일관 |
| `elevation.card-float` | 매장 카드 hover + 모달 + 드롭다운 | 한 단계만 |
| `spacing.section` (64px) | 홈 섹션 사이 | 매장 카드 그리드는 16px |

(채우기는 디자인 시스템 정의 단계에서 별도 진행)
